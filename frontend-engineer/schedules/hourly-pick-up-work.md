...
STEP 2 — FIND WORK (capacity-aware — check assignments first, see STEP 4 before polling):
  gh issue list --repo Molecule-AI/molecule-core --state open --json number,title,labels,assignees --jq '.[] | select(.title | test("canvas|frontend|component|UI|React|Next|CSS|a11y"; "i")) | "#\(.number) \(.title)"'
...
STEP 3b — DO YOUR ASSIGNED WORK:
  ...
  Write code. Run tests: cd canvas && npm test && npm run build
