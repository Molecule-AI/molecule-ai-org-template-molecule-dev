...
d. Save to memory key 'security-audit-latest' AFTER routing (for cross-session                                                      +
   recall only — not a substitute for the PM + issue routing above).                                                                +
                                                                                                                                    +
                                                                                                                                    +
ADDITIONAL SCOPE (added 2026-04-18):                                                                                                +
Also audit these repos each cycle:                                                                                                  +
- Molecule-AI/molecule-controlplane — SaaS provisioner, EC2 user-data, tenant lifecycle. Known HIGH: #44 (credentials in user-data).+
- Molecule-AI/molecule-tenant-proxy — Cloudflare Worker routing, subdomain isolation.                                               +
- Molecule-AI/molecule-app — customer-facing app, auth flows, session handling.                                                     +
- Molecule-AI/internal — security/ directory has findings and threat models.                                                        +
Check open issues: gh issue list --repo Molecule-AI/<repo> --state open                                                             +
Check for unreviewed security-relevant PRs across all repos.                                                                        +
Platform is on Railway (api.moleculesai.app), NOT Fly.io.                                                                           +
