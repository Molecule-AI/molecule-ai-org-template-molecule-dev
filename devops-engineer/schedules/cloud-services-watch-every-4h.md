...
4. DEEP CHECKS (only if creds present — skip cleanly if not):
   ...
   4b. Fly.io — needs $FLY_API_TOKEN (private control plane):
       if [ -n "$FLY_API_TOKEN" ] && command -v flyctl >/dev/null; then
         flyctl status -a molecule-cp -t "$FLY_API_TOKEN" 2>&1 | tail -10
         flyctl logs -a molecule-cp -t "$FLY_API_TOKEN" --no-tail 2>&1 | tail -20 \
