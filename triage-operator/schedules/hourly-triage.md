...
- Never skip hooks (--no-verify)                                                                                                   +
                                                                                                                                   +
ADDITIONAL REPO SCAN (added 2026-04-18):                                                                                           +
In STEP 1, also scan these repos for open issues and PRs:                                                                          +
- gh issue list --repo Molecule-AI/molecule-ai-workspace-runtime --state open --json number,title                                  +
- gh issue list --repo Molecule-AI/molecule-tenant-proxy --state open --json number,title                                          +
...
- gh issue list --repo Molecule-AI/molecule-ai-status --state open --json number,title                                             +
...
- gh issue list --repo Molecule-AI/internal --state open --json number,title                                                       +
                                                                                                                                   +
PR REVIEW ROUTING CHECK (added 2026-04-18 — fixes #959):                                                                           +
After STEP 2, for every open PR that has review comments:                                                                          +
- Check if ANY review comment is unaddressed (no reply from the PR author within 4h)                                               +
- If a QA or Security Auditor review is blocking AND no dev has responded, delegate_task to Dev Lead:                              +
  "PR #N has unaddressed blocking review from [qa-agent/security-agent]. Route to the PR author for fixes."                        +
- This ensures critical review findings don't sit unaddressed for hours.                                                           +
                                                                                                                                   +
INFRA NOTE: Platform runs on Railway (api.moleculesai.app). NOT Fly.io. Do not probe *.fly.dev URLs.                               +
Strategy docs (PLAN.md, CLAUDE.md) are in Molecule-AI/internal repo.
