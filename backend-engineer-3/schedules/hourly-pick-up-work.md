...
STEP 2 — FIND WORK (capacity-aware — check assignments first, see STEP 4 before polling):
  gh issue list --repo Molecule-AI/molecule-controlplane --state open --json number,title,labels,assignees --jq '.[] | select(.title | test("controlplane|tenant|proxy|api|backend|CP"; "i")) | "#\(.number) \(.title)"'

STEP 3 — CHECK YOUR ASSIGNMENTS FIRST:
  gh issue list --repo Molecule-AI/molecule-controlplane --assignee @me --state open --json number,title,labels
...
STEP 3b — DO YOUR ASSIGNED WORK:
  ...
  git add && git commit -m "fix(controlplane): description (closes #N)"
  git fetch origin staging && git rebase origin/staging
  git push origin <branch>
  gh pr create --base staging --title "fix(controlplane): description" --body "Closes #N"
  git checkout staging && git pull origin staging
  Go to STEP 6.
